    ...
    <div class="row g-3">
      <div class="col-md-2">
        ...
        <select name="tipo" class="form-select" id="tipoSelect" required>
          ...
          <option value="MP" @selected(old('tipo')==='MP')>Materia Prima</option>
          <option value="PT" @selected(old('tipo')==='PT')>Producto Terminado</option>
        </select>
      ...
      <div class="col-md-3">
        ...
        <select name="id_marca" class="form-select">
          ...
          @foreach ($marcas as $m)
            <option value="{{ $m->id_marca }}" @selected(old('id_marca')==$m->id_marca)>
              {{ $m->nombre }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-3 tipo-mp">
        ...
        <select name="id_mp_tipo" class="form-select">
          ...
          @foreach ($mpTipos as $t)
            <option value="{{ $t->id_mp_tipo }}" @selected(old('id_mp_tipo')==$t->id_mp_tipo)>
              {{ $t->nombre }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-3 tipo-pt">
        ...
        <select name="id_categoria_pt" class="form-select">
          ...
          @foreach ($categorias as $c)
            <option value="{{ $c->id_categoria_pt }}" @selected(old('id_categoria_pt')==$c->id_categoria_pt)>
              {{ $c->nombre }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-3 tipo-pt">
        ...
        <select name="id_subcategoria_pt" class="form-select">
          ...
          @foreach ($subcategorias as $s)
            <option value="{{ $s->id_subcategoria_pt }}" @selected(old('id_subcategoria_pt')==$s->id_subcategoria_pt)>
              {{ $s->nombre }}
            </option>
          @endforeach
        </select>
